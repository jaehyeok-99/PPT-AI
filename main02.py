...
# 출력 지침
스크립트는 반드시 아래의 4단계 구조를 따라야 합니다.

1.  **시작 요약**: 먼저, 프레젠테이션 전체 내용을 **한 문장으로 요약**하며 시작합니다. (예: 이 프레젠테이션은 드론의 기본 원리와 조종법을 다루는 입문 교육 자료입니다.)
2.  **전환 문구**: 그 다음, "이제 각 슬라이드의 내용을 상세히 요약해드리겠습니다." 라는 고정된 문장을 말합니다.
3.  **슬라이드별 상세 설명**: 이어서, "n번 슬라이드에서는..." 과 같이 슬라이드 번호를 언급하며 각 슬라이드의 핵심 내용을 순서대로 설명합니다. 내용이 비슷한 슬라이드는 "n번부터 m번까지는..." 과 같이 묶을 수 있습니다.
4.  **마무리 문구**: 모든 설명이 끝나면, "이상으로 요약을 마칩니다." 라는 고정된 문장으로 스크립트를 끝냅니다.

# 준수사항
* 문장은 최대한 간결하게, 핵심 내용 위주로 구성합니다.
* `###` 와 같은 헤더나 글머리 기호(`*`, `-`)는 절대 사용하지 마세요.
* 오직 하나의 완성된 줄글로만 결과물을 생성해야 합니다.
...
def extract_text_from_ppt(ppt_path):
    ...
    if not os.path.exists(ppt_path):
        return f"오류: '{ppt_path}' 파일을 찾을 수 없습니다."
    try:
...
def summarize_with_ollama(text):
    ...
    print(f" ({MODEL_NAME})를 호출하여 요약을 시작...")
    ...
    try:
        ...
        return f"Ollama API 호출 중 오류가 발생했. Ollama가 백그라운드에서 실행 중인지, 방화벽이 허용하는지 확인 \n오류: {e}"
